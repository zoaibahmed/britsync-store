...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setErrorMsg(data.error || 'Unauthorized Admin credentials.');
        return;
      }
    ...
    <main style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          <span style={{ fontSize: '0.8rem', letterSpacing: '2px', opacity: 0.6, textTransform: 'uppercase' }}>Secure Admin Gate</span>
        </div>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          <div>
            <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '0.5rem', opacity: 0.8 }}>Admin Identifier</label>
            <input 
              type="email" 
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '0.5rem', opacity: 0.8 }}>Access Key</label>
            <input 
              type="password" 
              placeholder="••••••••••••"
              value={password}
          ...
          >
            {loading ? 'Authorizing Access Key...' : 'Authenticate Admin'}
          </button>
        </form>
      </div>
    </main>
