...
export default async function MakerProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const mappedMaker = {
    ...maker,
    country: countryName,
    coverImage: maker.coverImageUrl || maker.coverMedia?.storageKey || 'https://images.unsplash.com/photo-1544816155-12df9643f363?auto=format&fit=crop&q=80&w=1600',
    founderPhoto: maker.founderPhotoUrl || maker.founderMedia?.storageKey || 'https://images.unsplash.com/photo-1570114668478-439564cbacda?auto=format&fit=crop&q=80&w=800',
    founderName: maker.founderName || maker.user?.name || 'Master Artisan',
    founderStory: maker.founderBiography || maker.heritageOriginStory || 'Dedicated to preserving local heritage and craft traditions.',
    businessStory: maker.heritageOriginStory || maker.founderBiography || 'A multi-generational craft workshop.',
    mission: maker.craftPhilosophy || 'To preserve traditional craftsmanship and build a legacy.',
    impactStory: 'Creating sustainable local jobs for generational artisans.',
    workshopGallery: JSON.stringify([maker.workshopPhoto1, maker.workshopPhoto2, maker.workshopPhoto3].filter(Boolean)),
    teamPhotos: '[]',
    productionPhotos: '[]',
    lifestylePhotos: '[]'
  };
